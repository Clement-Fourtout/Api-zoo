...
app.post('/contact', async (req, res) => {
  ...
  app.post('/send-response', async (req, res) => {
    ...
    if (!messageId || !responseText) {
        return res.status(400).json({ message: 'ID du message et texte de la réponse sont requis.' });
    }

    try {
        // Récupérer les détails du message à partir de la base de données
        ...
        pool.query(query, [messageId], async (err, results) => {
            ...
            const mailOptions = {
                from: process.env.OUTLOOK_EMAIL,
                to: contact.email, // L'adresse du visiteur
